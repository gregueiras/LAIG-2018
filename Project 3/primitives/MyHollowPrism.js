...
class MyHollowPrism extends CGFobject {
  constructor(scene, slices, stacks, factor) {
    super(scene);
  ...
  initVIN() {
    ...
    const outerVertices = this.vertices.length;
    for (var stack = 0; stack < this.stacks; stack++) {

      for (var slice = 0; slice < this.slices; slice++) {
        this.vertices.push(Math.cos((slice + 1) * this.delta) * this.factor);
        this.vertices.push(Math.sin((slice + 1) * this.delta) * this.factor);
        this.vertices.push(stack * alt);

        this.vertices.push(Math.cos(slice * this.delta) * this.factor);
        this.vertices.push(Math.sin(slice * this.delta) * this.factor);
        this.vertices.push(stack * alt); 
